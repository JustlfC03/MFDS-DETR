...
def main(args):
    ...
    name = "0092"
    im = Image.open("/root/autodl-tmp/ZJHospital/val2017/{}.jpg".format(name))
    if args.resume:
    ...
    print(enc_sampling_localtion[:,8888,2,3:,:])
    p = [[1034,923],[3263,1630],[3233,2262]]
    c = ["r", "m", "g", "b"]
    ...
    for idx, (xmin, ymin, xmax, ymax) in zip(keep.nonzero().cpu().numpy(), bboxes_scaled.cpu().numpy()):
        ...
        plt.figure()
        plt.imshow(im)
        #print(dec_reference_points[:,idx,:,:])
        # encoder
        # for head in range(8):
        #     stride = 4 * 3.75
        #     for i in range(4):
        ...
        #
        #         for j in range(4):
        #             k = sampling_locations[:,int(g),head,j,:,:].cpu().numpy()
        #             k = up_scale(k)
        ...
        for head in range(8):
            for i in range(4):
                for j in range(4):
                    k = sampling_locations[:,idx[0],head,j,:,:].cpu().numpy()
                    k = up_scale(k)
                    weights = dec_weights[:, idx[0], head, j, :]
                    #print(weights)
                    #plt.scatter(k[:,:,0]*4000, k[:,:,1]*3000, marker='o', c=c[j])
