...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='output/ZJHospital/fine-tune/LISC/checkpoint0099.pth', help='resume from checkpoint')
...
def main(args):
    ...
    names = ["0194", "06-0006", "0031", "188", "100", "0042", "123-0004", "192", "0012", "0107"]
    for name in names:
        im = Image.open("/root/autodl-tmp/ZJHospital/train2017/{}.jpg".format(name))
        src_file = f'/root/autodl-tmp/ZJHospital/train2017/{name}.jpg'
        dis_file = f'visualization/Decoder/{name}.jpg'
        shutil.copy(src_file, dis_file)
        if args.resume:
        ...
        dec_sampling_offsets = []
        dec_weights = []
        hooks = [
            model.transformer.decoder.layers[-1].cross_attn.sampling_offsets.register_forward_hook(lambda self, input, output: dec_sampling_offsets.append(output)),
            model.transformer.decoder.layers[-1].cross_attn.attention_weights.register_forward_hook(lambda self, input , output: dec_weights.append(output)),
            model.transformer.reference_points.register_forward_hook(lambda self, input, output: dec_reference_points.append(output))
        ...
        dec_weights = dec_weights[0]
        _, len_q, _ = dec_sampling_offsets[0].shape
        dec_weights = dec_weights.view(1, len_q, 8, 4*4)
        #计算decoder的权重信息
        dec_weights = F.softmax(dec_weights, -1).view(1, len_q, 8, 4, 4)
        dec_reference_points = dec_reference_points[0].sigmoid()
        dec_reference_points = dec_reference_points[:,:,None]*valid_ratios
        points = dec_sampling_offsets[0].view(1, len_q, 8, 4, 4, 2)
        spatial_shapes = torch.Tensor(spatial_shapes).to(device)
        offset_normalizer = torch.stack([spatial_shapes[..., 1], spatial_shapes[..., 0]], -1)
        sampling_locations = dec_reference_points[:, :, None, :, None, :] \
                             + points / offset_normalizer[None, None, None, :, None, :]
        ...
        for idx, (xmin, ymin, xmax, ymax) in zip(keep.nonzero().cpu().numpy(), bboxes_scaled.cpu().numpy()):
            ...
            for head in range(8):
                for i in range(4):
                    k = sampling_locations[:,idx[0],head,i,:,:].cpu().numpy()
                    k = up_scale(k)
                    cell_points.extend(k[0].tolist())
                    weights = dec_weights[:, idx[0], head, i, :].cpu().numpy()
                    p_weights.extend(weights[0].tolist())
                    #print("第{}个尺度的图对应的权重{}".format(j,dec_weights[:, idx[0], head, j,:]))
            cell_points = np.array(cell_points)
            p_weights = np.array(p_weights)
            #norm = Normalize(vmin=p_weights.min(), vmax=p_weights.max())
            #weights_norm = norm(p_weights)
            sorted_idx = np.argsort(p_weights)[::1]
            sorted_points = cell_points[sorted_idx]
            sorted_weights = p_weights[sorted_idx]
            #colors = ['b', 'g', 'm', 'r']
            #cmap = ListedColormap(colors)
            plt.scatter(sorted_points[:,0]*4000, sorted_points[:,1]*3000, marker='o', c=sorted_weights, cmap='jet')
            #plt.colorbar()
            b = plt.Rectangle(
            ...
            plt.xticks([])
            plt.yticks([])
            plt.savefig("visualization/Decoder/{}/{}_dec_vis_{}.jpg".format(dataset_name, name, index), bbox_inches = 'tight', dpi=400)
